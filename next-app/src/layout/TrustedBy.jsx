      ...
      <div className="w-full flex flex-col gap-10">
        ...
        <div className="relative w-full overflow-hidden">
          <div className="flex animate-scroll whitespace-nowrap gap-20">
            {firstRow.map((src, i) => (
              <img key={i} src={src?.src || src} alt={`Logo ${i}`} className="h-12 object-contain inline-block" />
            ))}
            {/* duplicate once for seamless loop */}
            {firstRow.map((src, i) => (
              <img key={`dup1-${i}`} src={src?.src || src} alt={`Logo dup ${i}`} className="h-12 object-contain inline-block" />
            ))}
          </div>
        </div>
        ...
        <div className="relative w-full overflow-hidden">
          <div className="flex animate-scroll-rev whitespace-nowrap gap-20">
            {secondRow.map((src, i) => (
              <img key={`rev-${i}`} src={src?.src || src} alt={`Logo reverse ${i}`} className="h-12 object-contain inline-block" />
            ))}
            {secondRow.map((src, i) => (
              <img key={`revdup-${i}`} src={src?.src || src} alt={`Logo reverse duplicate ${i}`} className="h-12 object-contain inline-block" />
            ))}
          </div>
        </div>
      </div>
